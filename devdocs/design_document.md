...
**Rationale:** Reliability is a hard constraint. OpenRouter provides a unified API surface for multiple providers. The chain `deepseek/deepseek-v4-flash` → `/openrouter/free` ensures availability during rate limits, provider outages, or quota exhaustion. The router is implemented with exponential backoff and circuit-breaker logging. This eliminates single-point-of-failure risks while maintaining cost predictability.
